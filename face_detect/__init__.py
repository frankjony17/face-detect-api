...
def create_app():
    ...
    # Error handler
    from face_detect.util.response_error import error_handler
    app.register_blueprint(error_handler)
    # Helpers controller
    from face_detect.util.helpers_controller import helpers
    app.register_blueprint(helpers)
    # Start controller
    from face_detect.controller.start_controller import start_route
    app.register_blueprint(start_route)
    # Detection controller
    from face_detect.controller.detection_controller import detection_route
    app.register_blueprint(detection_route)
    # Logger
    logging.getLogger('face_detect.init').info('Application created, ready to up.')
    return app
